...
const string strMessageMagic = "Lusocoin Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uLUC
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 LUC if made of 50 LUC inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
void ThreadScriptCheck() {
    RenameThread("lusocoin-scriptch");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LusocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LusocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LusocoinMiner(CWallet *pwallet)
{
    printf("LusocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lusocoin-miner");
    ...
    try { loop {
        ...
        printf("Running LusocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LusocoinMiner terminated\n");
        throw;
    }
}

void GenerateLusocoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LusocoinMiner, pwallet));
}
