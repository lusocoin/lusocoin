...
const int LUSOCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString LUSOCOIN_IPC_PREFIX("lusocoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(LUSOCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(LUSOCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(LUSOCOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click lusocoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start lusocoin: click-to-pay handler");
    else
...
{
    // clicking on lusocoin: URLs creates FileOpen events on the Mac:
