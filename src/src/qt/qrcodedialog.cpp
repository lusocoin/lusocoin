...
{
    QString ret = QString("lusocoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LUC unit input in lnReqAmount, we generate the URI with LUC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(LusocoinUnits::format(LusocoinUnits::LUC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
